  ...
  async getTokenUtxos (address, tokenId, utxoDelay) {
    try {
      ...
      */
      ...
      if (process.env.VERBOSE_LOG === 1) console.log(`utxos: ${JSON.stringify(utxos, null, 2)}`)
      ...
      if (process.env.VERBOSE_LOG === 1) console.log(`hydratedUtxos: ${JSON.stringify(hydratedUtxos, null, 2)}`)

      // Filter out the UTXOs that represent PSF tokens.
      let matchedUtxos
      if (tokenId) {
        matchedUtxos = hydratedUtxos.slpUtxos[0].utxos.filter(
  ...
  async agMerit (address, tokenId, utxoDelay) {
    try {
      ...
      */
      ...
      if (process.env.VERBOSE_LOG === 1) console.log(`agMerit address: ${address} tokenId: ${tokenId}`)
